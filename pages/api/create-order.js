...
export default async function handler(req, res) {
  ...
  try {
    // Step 1: Create the order in WooCommerce. It will correctly calculate the total with discounts.
    const wcResponse = await fetch(`${WC_API_URL}/wp-json/wc/v3/orders`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    // Step 3: Use the final total from the created WooCommerce order for Razorpay.
    // This ensures discounts are correctly applied. Convert to paise.
    const finalAmountInPaise = Math.round(parseFloat(wcOrder.total) * 100);
    ...
    // Step 4: Return the IDs to the frontend
    res.status(200).json({
      id: wcOrder.id,
      total: wcOrder.total,
      razorpay_order_id: rzpOrder.id
