    ...
    <>
      ...
      <div className="bg-white">
        ...
        <div className="section-padding container">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12">
            {authors.map((author) => (
              <Link key={author.id} href={`/shop?author=${author.slug}`}>
...
export async function getStaticProps() {
  try {
    ...
    const authorAttr = attributes.find(attr => attr.name === 'Book Author');
    
    let authors = [];
    if (authorAttr) {
      const authorTerms = await api.getAttributeTerms(authorAttr.id, { per_page: 100, orderby: 'name', order: 'asc' });
      authors = authorTerms.map(term => ({
        id: term.id,
        name: term.name,
        slug: term.slug,
        role: "Author",
        image: "https://images.pexels.com/photos/753695/pexels-photo-753695.jpeg",
        description: `Explore all books by ${term.name}`
      }));
    }
